...
saveRDS(seuList, file = "results/1dlpfc_seuList4.RDS")

print(sprintf("Total cells to process: %s", format(total_cells, big.mark = ",")))
print(sprintf("Total genes: %s", format(total_genes, big.mark = ",")))
print(sprintf("Number of datasets: %d", length(name_ID4)))
...
saveRDS(PRECASTObj, file = "results/1dlpfc_PRECASTObj.rds")
...
print(seuInt)
saveRDS(seuInt, file = "results/1dlpfc_seuInt.rds")
...
write_json(benchmark_results, "results/precast_benchmark1.json", pretty = TRUE, auto_unbox = TRUE)
...
library(Seurat)
seuInt<- readRDS("results/1dlpfc_seuInt.rds")
sce <- as.SingleCellExperiment(seuInt)
zellkonverter::writeH5AD(sce, "results/1dlpfc_seuInt.h5ad")
...
import os
import numpy as np
adata = sc.read_h5ad('results/1dlpfc_seuInt.h5ad')
...
adata.write('results/1dlpfc_seuInt_with_all_spatial.h5ad')
